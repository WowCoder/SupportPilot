...
class BM25Tool(BaseTool):
    ...
    def __init__(self, documents: List[Dict[str, Any]] = None):
        ...
        Args:
            documents: List of documents with 'content' and 'metadata' keys.
                      If not provided, documents will be loaded from ChromaDB.
        """
    ...
    def _ensure_index(self) -> bool:
        """
        Build BM25 index from ChromaDB if documents are available.
        ...
        # If documents were provided directly, build index from them
        if self._documents:
            self._build_index()
            self._index_built = True
            return True

        # Try to load documents from ChromaDB
        try:
            import chromadb
            ...
            if all_docs and all_docs.get('documents'):
                ...
                logger.warning('BM25: No documents found in ChromaDB')
                return False

        except Exception as e:
            logger.error(f'BM25: Failed to load documents from ChromaDB: {e}')
            return False
